function population = myGA(f,V,M,lb,ub)
% myGA(f,V,M,lb,ub)
% f : function to minimize
% V : Dimension of the search space.
% sM : Number of objectives.
% lb = lower bound vector.
% ub = upper bound vector. 

%% DEFINITION OF THE PARAMETERS

N = 50;    % Population size
NP = 50;    % Size of the mating pool
NC = 100;    % Number of children generated by generation
P = 0.9;     % probability of recombination
eta=0.08;
verbose = true; % (true) plots each iteration

%% GENETIC ALGORITHM

% Generation of the intial population
population=initPopulation(N,V);

population=evaluatePopulation(population,f,V,M,lb,ub);

population=sortPopulation(population,V,M);

% Main loop
it = 1;
while stopCriterion(it)

    parents = selectionTournament(population,NP,V,M);	
    
    offspring = geneticOperators(parents,NC,P,V,M,f,lb,ub,eta);
    
    population = [ population(:,1:V+M) ; offspring(:,1:V+M) ];
    
    population = sortPopulation(population,V,M);
    
    population = cropPopulation(population,N,V,M);
    
  %  population = [population_n ; population(floor(N/2):floor(N/2)+5,1:V+M)];
    % Visualization
    if verbose
        illustratePopulation(population,V,M,lb,ub,it);
        drawnow;
        pause(0.1);
    % pause
    end
        
    it = it+1;
end


end
